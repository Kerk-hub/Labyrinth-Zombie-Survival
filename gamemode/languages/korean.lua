...
LANGUAGE.wave_x_is_over_sub =
	"좀비들의 습격이 지연됐고, 상점에서 %d%% 할인된 가격으로 물품을 판매중이다."
...
LANGUAGE.class_not_unlocked_will_be_unlocked_x =
	"해당 좀비는 아직 풀려나지 않았다. %d웨이브 이후에 풀려난다."
...
LANGUAGE.press_f2_for_the_worth_menu = "F2를 눌러 Worth 메뉴 사용!"
...
LANGUAGE.exit_destroyed_only_one_remain_h =
	"마지막 하나만이 남았다! 만약 이마저 부서진다면 더 이상의 희망은 없다!"
...
LANGUAGE.controls_fast_zombie =
	"> 공격 1: 할큄\n> 공격 2: 달려듦 / 벽 타기 (벽 앞에서)\n> 재장전: 울부짖음"
LANGUAGE.controls_bloated_zombie =
	"> 공격 1: 할큄\n> 공격 2: 신음\n> 달리기: 죽은척 함\n> 죽을 때: 독 쐐기"
...
LANGUAGE.help_cont_introduction =
	[[<p>    좀비 대재앙을 경험할 수 있게 해주는 좀비 서바이벌에 오신 것을 환영한다! 여러분은 이제 바리케이드를 짓고, 좀비를 박살 내며, 언데드들과 전쟁을 벌일 수 있다.</p>
...
그 대신, 한 좀비는 2명의 인간을 죽여 성불(환생)할 수 있다. 생존하여 승리할 수 있는 두 번째 기회를 잡게 되는 겁니다.
...
LANGUAGE.help_cont_barricading =
	[[<p>바리케이딩은 생존에 절대로 없어서는 안 될 행위이다. 좀비들은 처음엔 약해 보이나 후반으로 갈수록 순식간에 팀 전체를 죽일 수 있을 정도로 강력해지기 때문이다.</p>
...
LANGUAGE.help_cont_upgrades =
	[[<p>포인트는 좀비를 죽이거나, 팀을 치료하거나, 터렛의 탄환을 충전하거나, 포인트샵을 운영하거나, 보급상자를 설치하거나 바리케이드를 수리함으로써 얻을 수 있다.
