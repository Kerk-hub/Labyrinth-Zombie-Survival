...
local DamageFloaters = CreateClientConVar("labyrinth_zs_damagefloaters", "1", true, false):GetBool()
cvars.AddChangeCallback("labyrinth_zs_damagefloaters", function(cvar, oldvalue, newvalue)
	...
	if wave < GAMEMODE:GetNumberOfWaves() and wave > 0 then
		...
		if MySelf:IsValid() and P_Team(MySelf) == TEAM_HUMAN then
			if MySelf:GetZSSPRemaining() > 0 then
				GAMEMODE:CenterNotify(
					translate.Format("unspent_skill_points_press_x", input.LookupBinding("gm_showspare1") or "F3")
				)
			end
